...
function convertToWiki(px) {
	var rv = "";
	Object.entries(px).forEach(([a, b]) => {
		rv += '<panel type="default" title="' + a + '" subtitle="" no-body="true">' + "\n";
		rv += '|<100% 30% 20% 40% 20% 10%>|' + "\n";
		rv += '^ Type ^ System(s) ^ Structure Name(s) ^ Type ^ Rigs ME ^ Rigs TE ^' + "\n";
		Object.entries(b).forEach(([c, d]) => {
			rv += '| ' + c + ' | ';
			rv += Array.from(d).map((xx) => xx.system).join(" \\\\ ") + ' | ';
			rv += Array.from(d).map((xx) => xx.name).join(" \\\\ ") + ' | ';
			rv += Array.from(d).map((xx) => xx.type).join(" \\\\ ") + ' | ';
			//rv += " | | \n";
			rv += d.map((xx) => xx.meRigged).join(" \\\\ ") + ' | ';
			rv += d.map((xx) => xx.teRigged).join(" \\\\ ") + ' | ';
			// <label type="warning">**T2**</label> \\ <label type="success">**T1**</label>
			rv += "\n";
		});
		rv += '</panel>' + "\n" + "\n";
	});
...
function mapStationsToPanels(systems) {
	...
	Object.entries(systems).forEach(([system, stations]) => {
		...
		stations.forEach(station => {
			...
			// @TODO ME/TE makes no sense for Repro, but let's go with it for now
			// same for the other non-manufacturing citadels
			const meRigged = rigs.some(rig => rig.includes('Material Efficiency') || rig.includes("Reprocessing Monitor") ||
							rig.includes("Reactor Efficiency") || rig.includes("Invention Optimization") || rig.includes("ME Research Optimization") ||
							rig.includes("Manufacturing Efficiency"))
				? rigs.some(rig => rig.endsWith(" I")) ? 'T1' : 'T2'
				: null;

			const teRigged = rigs.some(rig => rig.includes('Time Efficiency') || rig.includes("TE Research Optimization") || rig.includes("Manufacturing Efficiency"))
				? rigs.some(rig => rig.endsWith(" I")) ? 'T1' : 'T2'
				: null;
			...
			rigs.forEach(rig => {
				...
				if (structureRigs.manufacturing.ship[rigSize] && structureRigs.manufacturing.ship[rigSize].includes(rig)) {
					console.log(name, rigSize);
					if (rigSize == "xl") {
						panels["Ship Manufacturing"]["All Subcapitals"].push(structureInfo);
					} else {
						// this needs to split by ship type
						const where = panels["Ship Manufacturing"]["Specific"];
						const existingStructure = where.find(item => item.system === system && item.name === name);
						if (!existingStructure) {
							where.push(structureInfo);
						}
						//console.log(panels["Ship Manufacturing"]["Specific"]);
					}
				} else if (structureRigs.manufacturing.component[rigSize] && structureRigs.manufacturing.component[rigSize].includes(rig)) {
					const where = panels["Components"]["T1 Capital Components"];
					const existingStructure = where.find(item => item.system === system && item.name === name);
					if (!existingStructure) {
						where.push(structureInfo);
					}
				} else if (structureRigs.manufacturing.drone[rigSize] && structureRigs.manufacturing.drone[rigSize].includes(rig)) {
					const where = panels["Specialist Manufacturing"]["T1/T2 Drones & Fighters"];
					const existingStructure = where.find(item => item.system === system && item.name === name);
					if (!existingStructure) {
						where.push(structureInfo);
					}
				} else if (structureRigs.manufacturing.modulesRigs[rigSize] && structureRigs.manufacturing.modulesRigs[rigSize].includes(rig)) {
					const where = panels["Specialist Manufacturing"]["T1/T2 Modules & Rigs"];
					const existingStructure = where.find(item => item.system === system && item.name === name);
					if (!existingStructure) {
						where.push(structureInfo);
					}
				} else if (structureRigs.manufacturing.structure[rigSize] && structureRigs.manufacturing.structure[rigSize].includes(rig)) {
					const where = panels["Specialist Manufacturing"]["Structures"];
					const existingStructure = where.find(item => item.system === system && item.name === name);
					if (!existingStructure) {
						where.push(structureInfo);
					}
				} else if (structureRigs.manufacturing.charge[rigSize] && structureRigs.manufacturing.charge[rigSize].includes(rig)) {
					const where = panels["Specialist Manufacturing"]["Ammo"];
					const existingStructure = where.find(item => item.system === system && item.name === name);
					if (!existingStructure) {
						where.push(structureInfo);
					}
				...
				// Science Category
					...
					const where = panels["Science"]["T2 Invention"];
					const existingStructure = where.find(item => item.system === system && item.name === name);
					if (!existingStructure) {
						where.push(structureInfo);
					}
					...
					if (meRigged) {
						const where = panels["Science"]["Researching ME"];
						const existingStructure = where.find(item => item.system === system && item.name === name);
						if (!existingStructure) {
							where.push(structureInfo);
						}
					...
					if (teRigged) {
						const where = panels["Science"]["Researching TE"];
						const existingStructure = where.find(item => item.system === system && item.name === name);
						if (!existingStructure) {
							where.push(structureInfo);
						}
					...
					const where = panels["Science"]["BPC Copying"];
					const existingStructure = where.find(item => item.system === system && item.name === name);
					if (!existingStructure) {
						where.push(structureInfo);
					}
				...
				// Reaction Category
					...
					const where = panels["Reactions"]["All Reactions"];
					const existingStructure = where.find(item => item.system === system && item.name === name);
					if (!existingStructure) {
						where.push(structureInfo);
					}
				...
				// Reprocessing Category
					...
					const where = panels["Reprocessing"]["Asteroid Reprocessing"];
					const existingStructure = where.find(item => item.system === system && item.name === name);
					if (!existingStructure) {
						where.push(structureInfo);
					}
...
function runConversion() {
	...
	document.getElementById('wikiOutput').textContent = convertToWiki(panels);
}
